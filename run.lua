...
local function unravel(i)
	local x,y = 0,0
	for b=0,31 do
		if bit.band(1, bit.rshift(i, b)) == 1 then	-- set bit?
			if bit.band(b, 1) == 0 then	-- even bit index == x coord
				x = bit.bor(x, bit.lshift(1, bit.rshift(b, 1)))
			else	-- odd bit index == y coord
				y = bit.bor(y, bit.lshift(1, bit.rshift(b-1, 1)))
			end
		end
	end
	return x,y
...
for i=0,img.width * img.height do
	...
	local mask = bit.lshift(1, b) - 1	-- mask for 6 bits
	local p = 8 - b	-- how much to pad the channels
	img.buffer[0 + img.channels * (x + img.width * y)] = bit.lshift(bit.band(mask, i), p)
	img.buffer[1 + img.channels * (x + img.width * y)] = bit.lshift(bit.band(mask, bit.rshift(i, b)), p)
	img.buffer[2 + img.channels * (x + img.width * y)] = bit.lshift(bit.band(mask, bit.rshift(i, bit.lshift(b, 1))), p)
end
